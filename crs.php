...
define('CRS_DEFAULT_REGION', 404);//275824);
define('CRS_CHAPTER_GROUP_ID', 656);
...
function crs_civicrm_buildForm($formName, &$form) {
    ...
    if (!empty($settings)) {
      ...
      if ($settings['region_mode'] == CRS_REGION_USER) {
        ...
        if ($id = $session->get('region_contact_id', 'crs')) {
          $form->setDefaults(array('region_contact_id' => $id));
        }
      }
  ...
  elseif ($formName == 'CRM_Contribute_Form_Contribution_Confirm' || $formName == 'CRM_Contribute_Form_Contribution_ThankYou') {
...
function crs_civicrm_post($op, $objectName, $objectId, &$objectRef) {
  ...
  if (($op == 'create') && ($objectName == 'Contribution')) {
    ...
    //watchdog('crs', print_r($objectRef, true));

    // we only play with contributions made from pages with CRS settings
    if (($settings = $session->get('settings', 'crs')) && ($settings['contribution_page_id'] == $objectRef->contribution_page_id)) {
